...
#
# Testes de regressão de segurança — gerados no E03
# Execute: bundle exec rspec spec/requests/sessions_spec.rb
...
RSpec.describe "Sessions", type: :request do
  ...
  describe "POST /login" do
    ...
    # ─── TESTES DE SEGURANÇA (passarão após correção no E03) ───
    ...
    context "SQL injection attempts" do
      it "rejeita payload com comentário SQL (-- )" do
        ...
        # Deve retornar erro de autenticação, não redirecionar para home
  ...
  describe "POST /users" do
    context "mass assignment" do
      it "ignora o parâmetro role=admin" do
        ...
        expect(user.role).to eq("user")  # não "admin"
      end
      ...
      it "ignora o parâmetro credits com valor alto" do
        post "/users", params: {
          user: {
            ...
            email: "rico@evil.com",
            password: "senha123",
            credits: 999_999      # deve ser ignorado
          }
        }
        user = User.find_by(email: "rico@evil.com")
        expect(user.credits).to eq(0)   # valor padrão, não 999999
      end
    end
  end
end
  ...
  describe "GET /orders/:id" do
    context "IDOR attempt" do
      it "não permite que Alice acesse o pedido de Bob" do
        ...
        # Deve retornar 403 ou redirecionar, não mostrar o pedido
